...
int main()
{
    ...
    // double remainder = 0;
    // auto region = tflow_map->graph_->GetVertex(map->GetLaneletIDFromName("s1"))->state_;
    // auto fps = tflow_map->DecomposeTrafficRegion(region, remainder, 5.0);
    // RoadMapViz::ShowVehicleFootprints(fps, 10, "fp-decomp", false);

    auto fps = tflow_map->DiscretizeRoadNetwork(8.0);
    ...
    // auto fps = tflow_map->DecomposeCenterlines(lanelets, 3.0);
    ...
    // auto fps2 = tflow_map->DecomposeCenterlines(lanelets2, 3.0);
