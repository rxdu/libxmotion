...
enum class SquareCellLabel { OCCUPIED, FREE };
...
class SquareGridBase : public RectGridBase<SquareCellBase<T> *> {
 ...
 private:
  ...
  inline RectGridIndex IDToIndex(int64_t id) {
    return RectGridIndex(id % RectGridBase<SquareCellBase<T> *>::size_x_,
                         id / RectGridBase<SquareCellBase<T> *>::size_x_);
  }
};
