...
#include "xmnavigation/sampling/base/planner_base.hpp"
#include "xmnavigation/sampling/tree/kd_graph.hpp"
  ...
  PathType Search(std::shared_ptr<StateType> start,
    ...
    for (int32_t k = 0; k < iter_num; ++k) {
      ...
      if (BaseType::CheckPathValidity(nearest, new_state)) {
        ...
        BaseType::tree_.ConnectTreeNodes(nearest, new_state,
                                         nearest_to_new_dist);
        ...
        // 6. Make connections with vertices that lie within a ball of certain
        // radius
        double radius;
        ...
        for (auto &ns : near_states) {
          if (ns == nearest) continue;

          if (BaseType::CheckPathValidity(nearest, new_state)) {
            BaseType::tree_.ConnectTreeNodes(
                ns, new_state,
                BaseType::space_->EvaluateDistance(ns, new_state));
#ifdef SHOW_TREE_GROWTH
#ifdef SHOW_INTERMEDIATE_STEPS
