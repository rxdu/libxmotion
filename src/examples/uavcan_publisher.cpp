...
int main(int argc, const char** argv)
{
    ...
    {
        throw std::runtime_error("Failed to start the IMU publisher; error: " + std::to_string(imu_pub_init_res));
    }
    ...
    cmd_pub.setTxTimeout(uavcan::MonotonicDuration::fromMSec(1000));
    cmd_pub.setPriority(uavcan::TransferPriority::MiddleLower); 
    ...
    while (true)
    {
        ...
        // const int pub_res2 = imu_pub.broadcast(imu_msg);
        // if (pub_res2 < 0)
        // {
        ...
        if (pub_res3 < 0)
        {
            std::cerr << "IMU publication failure: " << pub_res3 << std::endl;
        }
    }
}