...
int main()
{
    ...
    /********** create road map **********/
    std::shared_ptr<RoadMap> road_map = std::make_shared<RoadMap>("/home/rdu/Workspace/librav/data/road_map/natick_roundabout_full.osm");
    ...
    auto ego_chn = traffic_map->GetTrafficChannel("s8", "s4");
    auto ego_flow = TrafficFlow(ego_chn);
    ego_flow.AssignConstantSpeedProfile(ego_id, 0.5);
    ...
    RoadMapViz::ShowCollisionFieldWithTrafficFlows(ego_polygon, cfield, &ego_flow, conflicting_flows, 10, "threat_field_roundabout2", true);
