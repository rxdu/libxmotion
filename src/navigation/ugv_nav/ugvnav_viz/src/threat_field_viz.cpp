...
void RoadMapViz::ShowCollisionFieldWithTrafficFlows(Polygon &ego, std::shared_ptr<CollisionField> cfield, TrafficFlow *ego_flow, std::vector<TrafficFlow *> &flows, int32_t pixel_per_unit, std::string window_name, bool save_img)
{
    RoadMapViz &viz = RoadMapViz::GetInstance();
    ...
    canvas = gdraw.DrawFilledPolygon(canvas, ego, false, CvDrawColors::magenta_color);
