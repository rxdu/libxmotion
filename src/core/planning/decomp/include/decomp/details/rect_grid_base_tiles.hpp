...
class GridTiles<T, std::enable_if_t<std::is_floating_point<T>::value || std::is_integral<T>::value>>
{
  protected:
    ...
    void ResizeGridTiles(int64_t x, int64_t y)
    {
        ...
        // std::cout << "old matrix" << old_matrix << std::endl;
        // std::cout << "resize: " << old_rows << " , " << old_cols << " , " << y << " , " << x << " ; " << overlap_rows << " , " << overlap_cols << std::endl;

        grid_tiles_.block(0, 0, overlap_rows, overlap_cols) = old_matrix.block(0, 0, overlap_rows, overlap_cols);
    }
