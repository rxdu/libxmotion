...
class SquareCell {
public:
  SquareCell(uint64_t id, uint32_t col, uint32_t row, BoundingBox bbox,
             OccupancyType occupancy)
      : occu_(occupancy), geo_mark_id_(0) {
    index_.x = col;
    index_.y = row;
  ...
  double GetHeuristic(const SquareCell &other_struct) const {
    ...
    // static_cast: can get wrong result to use "unsigned long" type for
    // deduction
...
public:
  ...
  SquareGrid(uint32_t col_num, uint32_t row_num, uint32_t cell_size,
             int64_t img_offset_x, int64_t img_offset_y);
  ~SquareGrid();
...
private:
  ...
  BoundingBox CalcBoundingBox(uint64_t id, int64_t img_offset_x,
                              int64_t img_offset_y);
...
public:
  ...
  std::vector<SquareCell *> GetNeighboursWithinRange(uint64_t id,
                                                     uint32_t cell_range);
};
}
