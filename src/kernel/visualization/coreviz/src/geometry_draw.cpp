...
void GeometryViz::DrawPolyline(CvCanvas &canvas, const Polyline &polyline, bool show_dot, cv::Scalar ln_color, int32_t thickness)
{
...
void GeometryViz::DrawCubicSpline(CvCanvas &canvas, const CSpline &spline, double step, cv::Scalar ln_color, int32_t thickness)
{
...
void GeometryViz::DrawParametricCurve(CvCanvas &canvas, const ParametricCurve &pcurve, double step, cv::Scalar ln_color, int32_t thickness)
{
    std::vector<cv::Point2d> pts;
...
void GeometryViz::DrawLabelPoint(CvCanvas &canvas, double x, double y, cv::Scalar ln_color, int32_t thickness)
{
    canvas.DrawPoint({x, y}, 2, ln_color, thickness);
}

void GeometryViz::DrawPolygon(CvCanvas &canvas, const Polygon &polygon, bool show_dot, cv::Scalar ln_color, int32_t thickness)
{
...
void GeometryViz::FillPolygon(CvCanvas &canvas, const Polygon &polygon, bool show_dot, cv::Scalar fill_color, cv::Scalar ln_color, int32_t thickness)
{
    std::size_t pt_num = polygon.GetPointNumer();
...
*/
void GeometryViz::DrawPolygonDirection(CvCanvas &canvas, const Polygon &polygon, cv::Scalar ln_color, int32_t thickness)
{
    ...
    canvas.DrawArrowedLine({center_x, center_y}, {front_x, front_y}, 0.3, ln_color, thickness);
}

void GeometryViz::WritePointPosition(CvCanvas &canvas, const std::vector<SimplePoint> &points)
{
    for (auto &pt : points)
    {
...
void GeometryViz::WriteTextAtPosition(CvCanvas &canvas, std::string txt, SimplePoint pt)
{
    canvas.WriteText(txt, {pt.x, pt.y});
}

void GeometryViz::DrawDistribution(CvCanvas &canvas, double cx, double cy, double xspan, double yspan, std::function<double(double, double)> dist_fun)
{
    ...
    */
    ...
    cv::Mat threat_vis = MatrixViz::CreateColorMapFromEigenMatrix(threat_matrix, true);
