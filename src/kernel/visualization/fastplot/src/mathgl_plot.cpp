...
void MathGLPlot::Run(MathGLPlotGraph_t plot_func, std::string window_name)
{
    ...
    // std::unique_ptr<mglFLTK> mgl_win = std::make_unique<mglFLTK>(func_ptr, window_name.c_str());
    // mgl_win->Run();

    mglQT mgl_win(func_ptr, window_name.c_str());
    mgl_win.Run();
}

void MathGLPlot::Run(mglDraw *draw, std::string window_name)
{
    // std::unique_ptr<mglFLTK> mgl_win = std::make_unique<mglFLTK>(draw, window_name.c_str());
    // mgl_win->Run();
    
    mglQT mgl_win(draw, window_name.c_str());
    mgl_win.Run();
}
