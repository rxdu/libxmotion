...
#include "asyncio/async_serial.hpp"
...
ASyncSerial::ASyncSerial(std::string device, unsigned baudrate, bool hwflow)
    : device_(device),
      baudrate_(baudrate),
      hwflow_(hwflow),
      tx_total_bytes(0),
...
void ASyncSerial::open(std::string device, unsigned baudrate, bool hwflow) {
  ...
  std::cout << "connection: " << conn_id << " , device: " << device_ << " @ "
  ...
  try {
    ...
    serial_dev_.set_option(SPB::flow_control(
...
void ASyncSerial::iostat_tx_add(size_t bytes) { tx_total_bytes += bytes; }

void ASyncSerial::iostat_rx_add(size_t bytes) { rx_total_bytes += bytes; }

void ASyncSerial::send_bytes(const uint8_t *bytes, size_t length) {
  if (!is_open()) {
    std::cerr << "send: channel closed! connection id: " << conn_id
              << std::endl;
    return;
  }
...
void ASyncSerial::call_receive_callback(uint8_t *buf, const std::size_t bufsize,
...
void ASyncSerial::default_receive_callback(uint8_t *buf, const size_t bufsize,
...
void ASyncSerial::do_read(void) {
  ...
  serial_dev_.async_read_some(
      buffer(rx_buf), [sthis](error_code error, size_t bytes_transferred) {
        ...
        sthis->call_receive_callback(sthis->rx_buf.data(), sthis->rx_buf.size(),
                                     bytes_transferred);
        sthis->do_read();
      });
...
void ASyncSerial::do_write(bool check_tx_state) {
  ...
  serial_dev_.async_write_some(
      ...
      [sthis, &buf_ref](error_code error, size_t bytes_transferred) {
        ...
        if (buf_ref.nbytes() == 0) sthis->tx_q.pop_front();
...
//     url_parse_host(path, file_path, baudrate, ASyncSerial::DEFAULT_DEVICE,
//     ASyncSerial::DEFAULT_BAUDRATE); url_parse_query(query);
...
//         std::cout << "URL: " << url.c_str() << " looks like file path" <<
//         std::endl; return url_parse_serial(url, "", false);
//     }
