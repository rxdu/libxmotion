...
#include "asyncio/device_error.hpp"
#include "asyncio/msg_buffer.hpp"

namespace librav {
using steady_clock = std::chrono::steady_clock;
using lock_guard = std::lock_guard<std::recursive_mutex>;

/// Note: instance of ASyncSerial MUST be managed by a
/// std::shared_ptr<ASyncSerial>
class ASyncSerial : public std::enable_shared_from_this<ASyncSerial> {
 public:
   ...
   * @param bufsize: size of the buffer (the value should be constant in most
   * cases)
   * @param bytes_received: number of bytes received inside the buffer
   */
  using ReceiveCallback = std::function<void(uint8_t *buf, const size_t bufsize,
 ...
 public:
  ASyncSerial(std::string device = DEFAULT_DEVICE,
  ...
  void open(std::string device = "", unsigned baudrate = 0,
            bool hwflow = false);
  void close();

  void set_baud(unsigned baudrate) {
    serial_dev_.set_option(asio::serial_port_base::baud_rate(baudrate));
  }
 ...
 private:
  ...
  void call_receive_callback(uint8_t *buf, const std::size_t bufsize,
                             std::size_t bytes_received);
  void default_receive_callback(uint8_t *buf, const std::size_t bufsize,
                                std::size_t bytes_received);
