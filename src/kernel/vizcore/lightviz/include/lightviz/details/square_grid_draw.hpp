...
namespace librav
{
class SquareGridDraw
{
  public:
    SquareGridDraw(CartesianCanvas &canvas) : canvas_(canvas){};

    // basic drawing
    template <typename GridType>
    void SetupCanvas(GridType *grid, int32_t ppu = 100)
    {
        // create canvas
        int32_t vis_side_size = grid->GetCellSize() * ppu;
        canvas_.SetupCanvas(grid->SizeX() * vis_side_size, grid->SizeY() * vis_side_size, CvDrawColors::bg_color);
    }

    template <typename GridType>
    void DrawGridCell(GridType *grid)
    {
        // draw cells
        for (int32_t y = 0; y < grid->SizeY(); ++y)
            for (int32_t x = 0; x < grid->SizeX(); ++x)
            {
                ...
                if (cell->label == SquareCellLabel::OCCUPIED)
                {
                    ...
                    canvas_.paint_area(rngy, rngx) = CvDrawColors::obs_color;
                }
            }
    }
    ...
    void DrawGridNet(GridType *grid)
    {
        ...
        for (int32_t y = 1; y < grid->SizeY(); ++y)
        {
            ...
            cv::Point pt1(first_vertex.x * canvas_.ppu_, first_vertex.y * canvas_.ppu_);
            cv::Point pt2(last_vertex.x * canvas_.ppu_, last_vertex.y * canvas_.ppu_);

            CvDraw::DrawLine(canvas_.paint_area, pt1, pt2);
        }
    ...
    {
        cv::Range srngx(path.front()->vertices[0].x * canvas_.ppu_, path.front()->vertices[1].x * canvas_.ppu_);
        cv::Range srngy(path.front()->vertices[0].y * canvas_.ppu_, path.front()->vertices[2].y * canvas_.ppu_);
        canvas_.paint_area(srngy, srngx) = CvDrawColors::start_color;

        cv::Range grngx(path.back()->vertices[0].x * canvas_.ppu_, path.back()->vertices[1].x * canvas_.ppu_);
        cv::Range grngy(path.back()->vertices[0].y * canvas_.ppu_, path.back()->vertices[2].y * canvas_.ppu_);
        canvas_.paint_area(grngy, grngx) = CvDrawColors::finish_color;
    }
    ...
    void DrawGridPath(const std::vector<GridCellType *> &path)
    {
        for (int i = 0; i < path.size() - 1; ++i)
        {
            ...
            cv::Point pt1(first_cell.x * canvas_.ppu_, first_cell.y * canvas_.ppu_);
            cv::Point pt2(next_cell.x * canvas_.ppu_, next_cell.y * canvas_.ppu_);

            CvDraw::DrawLine(canvas_.paint_area, pt1, pt2, CvDrawColors::intermediate_color);
        }
    }
    ...
    void DrawGridGraph(GridType *grid, Graph<GridCellType *> *graph)
    {
        ...
        {
            auto edge = *edge_it;
            int64_t loc_x1 = edge.src_->state_->center.x * canvas_.ppu_;
            int64_t loc_y1 = edge.src_->state_->center.y * canvas_.ppu_;
            int64_t loc_x2 = edge.dst_->state_->center.x * canvas_.ppu_;
            int64_t loc_y2 = edge.dst_->state_->center.y * canvas_.ppu_;

            CvDraw::DrawLine(canvas_.paint_area, cv::Point(loc_x1, loc_y1), cv::Point(loc_x2, loc_y2), cv::Scalar(237, 149, 100));
        }
        ...
        for (auto vertex = graph->vertex_begin(); vertex != graph->vertex_end(); ++vertex)
        {
            ...
            int32_t loc_x = vertex->state_->center.x * canvas_.ppu_;
            int32_t loc_y = vertex->state_->center.y * canvas_.ppu_;

            cv::Point center(loc_x, loc_y);
            CvDraw::DrawPoint(canvas_.paint_area, center);
