...
int main()
{
...

    {
        ...
        canvas.DrawCircle({300 / 600.0, 200 / 400.0}, 35, CvColors::yellow_color, 3);
        canvas.DrawLine({50 / 600.0, 100 / 400.0}, {550 / 600.0, 100 / 400.0});
        canvas.DrawLine({80 / 600.0, 50 / 400.0}, {520 / 600.0, 150 / 400.0}, CvColors::green_color, 2);
        canvas.DrawLine({80 / 600.0, 150 / 400.0}, {520 / 600.0, 50 / 400.0}, CvColors::orange_color, 2);
        canvas.DrawArrowedLine({300 / 600.0, 150 / 400.0}, {300 / 600.0, 50 / 400.0}, 0.3, CvColors::red_color, 3);

        canvas.DrawRectangle({20 / 600.0, 20 / 400.0}, {580 / 600.0, 380 / 400.0}, CvColors::purple_color, 2);

        std::vector<CPoint> polyline{{80 / 600.0, 200 / 400.0}, {300 / 600.0, 350 / 400.0}, {520 / 600.0, 200 / 400.0}};
        canvas.DrawPolyline(polyline, false, CvColors::palegreen_color, 5);
        std::vector<CPoint> cpolyline{{150 / 600.0, 200 / 400.0}, {300 / 600.0, 350 / 400.0}, {450 / 600.0, 200 / 400.0}};
        canvas.DrawPolyline(cpolyline, true, CvColors::silver_color, 5);

        canvas.DrawEllipse({300 / 600.0, 200 / 400.0}, cv::Size(150.0, 50.0), 135, 0, 360);
        canvas.DrawEllipse({300 / 600.0, 200 / 400.0}, cv::Size(150.0, 50.0), 45, 0, 360);

        std::vector<CPoint> fpolyline{{100 / 600.0, 250 / 400.0}, {50 / 600.0, 350 / 400.0}, {150 / 600.0, 350 / 400.0}};
        canvas.FillPoly(fpolyline, CvColors::navy_color);

        std::vector<CPoint> fpolyline2{{500 / 600.0, 250 / 400.0}, {450 / 600.0, 350 / 400.0}, {550 / 600.0, 350 / 400.0}};
        canvas.FillConvexPoly(fpolyline2, CvColors::navy_color);

        canvas.WriteText("demo0.5", {20 / 600.0, 200 / 400.0}, 0.5);
        canvas.WriteText("demo1.0", {50 / 600.0, 180 / 400.0}, 1.0, CvColors::cyan_color);
...

    {
        ...
        canvas2.DrawArrowedLine({30.0, 15.0}, {30.0, 5.0}, 0.3, CvColors::red_color, 3);
