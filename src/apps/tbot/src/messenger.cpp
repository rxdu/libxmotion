...
void Messenger::HandleCanFrame(const can_frame *rx_frame) {
  ...
  switch (rx_frame->can_id) {
    case TBOT_SUPERVISED_STATE_CAN_ID: {
      supervised_state_.supervised_mode =
          static_cast<SupervisedMode>(rx_frame->data[0]);
      break;
    }
    ...
    case TBOT_TARGET_RPM_CAN_ID: {
      ...
      rpm_buffers_[DataBufferIndex::kTargetRpmRight].AddPoint(t, right / 1.0f);
    }
  }
}
...
void Messenger::SendMotionCommand(float linear, float angular) {
  ...
  frame.data[0] = static_cast<uint8_t>(
      (static_cast<uint32_t>(linear_cmd) & 0xff000000) >> 24);
  frame.data[1] = static_cast<uint8_t>(
      (static_cast<uint32_t>(linear_cmd) & 0x00ff0000) >> 16);
  frame.data[2] = static_cast<uint8_t>(
      (static_cast<uint32_t>(linear_cmd) & 0x0000ff00) >> 8);
  frame.data[3] = static_cast<uint8_t>(
      (static_cast<uint32_t>(linear_cmd) & 0x000000ff) >> 0);

  frame.data[4] = static_cast<uint8_t>(
      (static_cast<uint32_t>(angular_cmd) & 0xff000000) >> 24);
  frame.data[5] = static_cast<uint8_t>(
      (static_cast<uint32_t>(angular_cmd) & 0x00ff0000) >> 16);
  frame.data[6] = static_cast<uint8_t>(
      (static_cast<uint32_t>(angular_cmd) & 0x0000ff00) >> 8);
  frame.data[7] = static_cast<uint8_t>(
      (static_cast<uint32_t>(angular_cmd) & 0x000000ff) >> 0);
