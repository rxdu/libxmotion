...
int main()
{
    ...
    canvas.DrawCurveFunction({-50, .1, 50}, [](double x) -> double { return cos(x * .628f + 4) * 5; },
                             CvColors::blue_color);
    ...
    // example from http://sites.millersville.edu/bikenaga/calculus/parametric-equations/parametric-equations.html
    canvas.DrawParametricCurve({-50, .1, 50},
                               [](double s) -> double { return s * s * s + s + 2; },
