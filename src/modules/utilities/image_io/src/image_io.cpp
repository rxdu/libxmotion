...
MonoImageMatrix ImageIO::ReadImage(std::string file_name) {
  ...
  std::cout << "read image size (w/x,h/y,d): " << x << " , " << y << " , " << n
            << std::endl;
  ...
  if (n <= 2) {
    ...
    for (int32_t j = 0; j < y; ++j)
      for (int32_t i = 0; i < x; ++i)
        image.data(j, i) = 0.299 * data[j * (x * n) + i * n] +
...
bool ImageIO::SaveToImage(const MonoImageMatrix &matrix,
  ...
  bool result;
  if (save_to_png)
    result = stbi_write_png(file_name.c_str(), matrix.w, matrix.h, matrix.d,
                            data, matrix.w);
  else
    result = stbi_write_jpg(file_name.c_str(), matrix.w, matrix.h, matrix.d,
                            data, 100);
  free(data);
