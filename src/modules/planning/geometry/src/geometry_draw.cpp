...
void GeometryViz::DrawCubicSpline(CvCanvas &canvas, const CSpline &spline,
                                  double step, cv::Scalar ln_color,
                                  int32_t thickness) {
...
void GeometryViz::DrawParametricCurve(CvCanvas &canvas,
                                      const ParametricCurve &pcurve,
                                      double step, cv::Scalar ln_color,
                                      int32_t thickness) {
  std::vector<cv::Point2d> pts;
...
void GeometryViz::DrawLabelPoint(CvCanvas &canvas, double x, double y,
                                 cv::Scalar ln_color, int32_t thickness) {
  canvas.DrawPoint({x, y}, 2, ln_color, thickness);
}

void GeometryViz::DrawPolygon(CvCanvas &canvas, const Polygon &polygon,
                              bool show_dot, cv::Scalar ln_color,
                              int32_t thickness) {
...
void GeometryViz::FillPolygon(CvCanvas &canvas, const Polygon &polygon,
                              bool show_dot, cv::Scalar fill_color,
                              cv::Scalar ln_color, int32_t thickness) {
  std::size_t pt_num = polygon.GetPointNumer();
 ...
 */
void GeometryViz::DrawPolygonDirection(CvCanvas &canvas, const Polygon &polygon,
                                       cv::Scalar ln_color, int32_t thickness) {
  if (polygon.GetPointNumer() < 3) return;
  ...
  canvas.DrawArrowedLine({center_x, center_y}, {front_x, front_y}, 0.3,
                         ln_color, thickness);
}

void GeometryViz::WritePointPosition(CvCanvas &canvas,
                                     const std::vector<SimplePoint> &points) {
  for (auto &pt : points) {
    std::string pos_str = "(" + std::to_string(static_cast<int32_t>(pt.x)) +
                          "," + std::to_string(static_cast<int32_t>(pt.y)) +
                          ")";
    canvas.WriteText(pos_str, {pt.x, pt.y});
  }
}

void GeometryViz::WriteTextAtPosition(CvCanvas &canvas, std::string txt,
                                      SimplePoint pt) {
  canvas.WriteText(txt, {pt.x, pt.y});
}

void GeometryViz::DrawDistribution(
    CvCanvas &canvas, double cx, double cy, double xspan, double yspan,
    std::function<double(double, double)> dist_fun) {
  ...
  if (dxmin < xmin) dxmin = xmin;
  if (dxmax > xmax) dxmax = xmax;
  if (dymin < ymin) dymin = ymin;
  if (dymax > ymax) dymax = ymax;
  ...
  {
      threat_matrix(k % y_size, k / y_size) = threat(dxmin + (k / y_size) / ppu,
  dymin + (k % y_size) / ppu, t_k);
  }
  */

  // TBB parallel version
  const auto &fill_threat_matrix2 = [&dist_fun, &threat_matrix, x_size, y_size,
                                     dxmin, dymin, ppu](size_t k) {
    threat_matrix(k % y_size, k / y_size) =
        dist_fun(dxmin + (k / y_size) / ppu, dymin + (k % y_size) / ppu);
  };
  tbb::parallel_for(size_t(0), size_t(pixel_num), fill_threat_matrix2);

  cv::Mat threat_vis =
      MatrixViz::CreateColorMapFromEigenMatrix(threat_matrix, true);

  // merge threat distribution to canvas
  auto top_left_pixel = canvas.ConvertGeometryPointToPixel(
      dxmin, dymax);  // y inverted in cartesian coordinate
  threat_vis.copyTo(canvas.GetPaintArea()(cv::Rect(
      top_left_pixel.x, top_left_pixel.y, threat_vis.cols, threat_vis.rows)));
}
