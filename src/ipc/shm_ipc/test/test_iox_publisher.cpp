...
int main() {
  ...
  while (!iox::posix::hasTerminationRequested()) {
    ...
    //  * The first argument of the callable must be T* and is the location that
    //  the callable should
    //      write its result to.
    ...
    publisher
        ...
        .or_else([](auto& error) {
          // Do something with error.
          std::cerr << "Unable to publishResultOf, error: " << error
                    << std::endl;
        });
    //! [API Usage #4]

    std::cout << APP_NAME << " sent values: " << sampleValue1 << ", "
              << sampleValue2 << ", " << sampleValue3 << ", " << ct << ", "
