...
#show: arkheion.with(
  ...
  abstract: [These notes collect the core modeling, estimation, and control formulations implemented in xmNavigation in one document, so that the shared conventions — coordinate frames, quaternion algebra, discrete-time propagation, and the scalar-templated numerical cores compiled by both the CPU and CUDA paths — carry a single consistent formulation across topics. The sections run lowest-level to highest: the Platform Models section catalogs the benchmark plants the estimator and controller are evaluated on, each paired with the closed-form oracle that validates it; the MEKF section derives the multiplicative extended Kalman filter for attitude and state estimation; and the MPPI section derives the information-theoretic model-predictive path-integral controller and records its CPU and GPU execution strategy. Standalone errata and implementation-review notes (e.g. the MEKF errata) are maintained separately.],
