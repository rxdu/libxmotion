...
The CUDA backend implements the same seam following the architecture of MPPI-Generic @Vlahov2024-generic: one thread per sample (the split-rollout arrangement — right for small-state models whose state fits in registers), and dynamics/cost as scalar-templated raw-span cores (`model_core` / `critic_core`) that *both* backends compile — the Eigen model/critic classes wrap the cores on the CPU; the device-side "program" POD calls them in float32. Device rollouts run in float because FP64 executes at 1/32 rate on GTX-class and Jetson Orin hardware; the seam stays double-typed (narrow on upload, widen on download), and CPU/GPU costs agree to float accumulation error (bounded by test at $10^{-3}$ relative, observed $tilde 10^{-5}$). Measured rollout-evaluation time on a GTX 1660 Ti vs the 8-core host, wheeled model, $T = 50$: $K = 2048$: 0.19 ms GPU / 2.9 ms CPU-serial / 0.66 ms CPU-8; $K = 32768$: 3.4 / 45.3 / 10.0 ms; $K = 131072$: 14.5 / 180 / 38.8 ms. With evaluation this cheap, end-to-end planning is bounded by host-side noise generation — moving sampling on-device (curand) is the next GPU step, together with the SRB program for full-order legged rollouts @Xue2024-dialmpc.
