...
int main(int argc,char* argv[])
{
	...
	auto defaultHandler = worker->addDefaultLogger("hummingbird_sim",
			home_path + "/Workspace/srcl_robot_suite/srcl_ctrl/control/log/quad");
...
//				std::cout << "laser size: "<< sim_process.GetRobotState().laser_points_.size() << std::endl;
				...
				// send robot state data to ROS network
...
//				vis_trans.SendQuadTransformToROS(sim_process.GetRobotState().position_, sim_process.GetRobotState().quat_);
				...
				vis_trans.SendRobotStateDataToROS(sim_process.GetRobotState());
			}
//			else
