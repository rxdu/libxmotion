...
import srcl_msgs.Point3Df_t

class LaserScanPoints_t(object):
    __slots__ = ["trans_sensor2world", "point_num", "points"]

    def __init__(self):
        self.trans_sensor2world = srcl_msgs.Pose_t()
        self.point_num = 0
        self.points = []
    ...
    def _encode_one(self, buf):
        assert self.trans_sensor2world._get_packed_fingerprint() == srcl_msgs.Pose_t._get_packed_fingerprint()
        self.trans_sensor2world._encode_one(buf)
        buf.write(struct.pack(">q", self.point_num))
        for i0 in range(self.point_num):
            assert self.points[i0]._get_packed_fingerprint() == srcl_msgs.Point3Df_t._get_packed_fingerprint()
    ...
    def _decode_one(buf):
        ...
        self.trans_sensor2world = srcl_msgs.Pose_t._decode_one(buf)
        self.point_num = struct.unpack(">q", buf.read(8))[0]
        self.points = []
        for i0 in range(self.point_num):
            self.points.append(srcl_msgs.Point3Df_t._decode_one(buf))
        return self
    ...
    def _get_hash_recursive(parents):
        ...
        tmphash = (0x503d7b918ac7be34+ srcl_msgs.Pose_t._get_hash_recursive(newparents)+ srcl_msgs.Point3Df_t._get_hash_recursive(newparents)) & 0xffffffffffffffff
