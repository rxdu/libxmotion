 ...
 * =====================================================================================
 *
 *       Filename:  test_ompl.cpp
 *
 *    Description:
 *
 *        Version:  1.0
 *        Created:  06/27/2016 04:15:57 PM
 *       Revision:  none
 *       Compiler:  gcc
 *
...
// Kinematic car model object definition.  This class does NOT use ODESolver to
...
bool isStateValid(const oc::SpaceInformation *si, const ob::State *state) {
	...
	se2state->as<ob::RealVectorStateSpace::StateType>(0);

    /// extract the second component of the state and cast it to what we expect
    const ob::SO2StateSpace::StateType *rot =
	se2state->as<ob::SO2StateSpace::StateType>(1);
...
class DemoControlSpace : public oc::RealVectorControlSpace {
   public:
    DemoControlSpace(const ob::StateSpacePtr &stateSpace)
...
void planWithSimpleSetup(void) {
	...
	&isStateValid, ss.getSpaceInformation().get(), std::placeholders::_1));

    // Setting the propagation routine for this space:
    // KinematicCarModel does NOT use ODESolver
    // ss.setStatePropagator(oc::StatePropagatorPtr(new
    // KinematicCarModel(ss.getSpaceInformation())));

    // Use the ODESolver to propagate the system.  Call
    // KinematicCarPostIntegration
    // when integration has finished to normalize the orientation values.
    oc::ODESolverPtr odeSolver(
	new oc::ODEBasicSolver<>(ss.getSpaceInformation(), &KinematicCarODE));
    ss.setStatePropagator(oc::ODESolver::getStatePropagator(
	odeSolver, &KinematicCarPostIntegration));
