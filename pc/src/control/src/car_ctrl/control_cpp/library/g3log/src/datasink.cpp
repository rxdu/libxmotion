...
#include "g3log/datasink.hpp"
#include "datasinkhelper.ipp"
#include <cassert>

namespace g3 {
   using namespace internal_data;


   DataSink::DataSink(const std::string &log_prefix, const std::string &log_directory)
   ...
   void DataSink::fileWrite(LogMessageMover message) {
      std::ofstream &out(filestream());
      out << message.get().toString();
   }

   std::string DataSink::changeLogFile(const std::string &directory) {
   ...
   std::string DataSink::fileName() {
      return _log_file_with_path;
   }
   void DataSink::addLogFileHeader() {
      filestream() << header();
   }
