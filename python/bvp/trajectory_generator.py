"""
Trajectory generator for a unicycle model
...
# Reference:
# [1] Kelly, Alonzo, and Bryan Nagy. 2003. "Reactive Nonholonomic
#     Trajectory Generation via Parametric Optimal Control." The
#     International Journal of Robotics Research 22 (7-8): 583-601.
#
# System model
...
terminal_residual = 0.001
residual_weight = np.array([1.0, 1.0, 1.0, 1.0])
index_set = []
res_set = []


def calculate_g(q, stf):

    st = get_state_at(q)
    gq = np.array([st.x - stf.x,
                   st.y - stf.y,
                   st.theta - stf.theta,
                   st.kappa - stf.kappa])
...
def calculate_jacobian(q):
    ...
    theta_f = a * sf + b * (sf**2) / 2.0 + c * \
        (sf**3) / 3.0 + d * (sf**4) / 4.0
    kappa_f = a + b * sf + c * (sf**2) + d * (sf**3)
    kappa_f_prime = b + 2 * c * sf + 3 * d * (sf**2)
    ...
    J[0, :] = [-x_p0/2.0, -x_p1/3.0, -x_p2/4.0, np.cos(theta_f)]
    J[1, :] = [-y_p0/2.0, -y_p1/3.0, -y_p2/4.0, np.sin(theta_f)]
...
def optimize_trajectory(st0, stf, q0, max_iter=3, show_animation=True):
    ...
    for i in range(max_iter):
        ...
        gq = calculate_g(q, stf)
        res = calculate_residual(gq)
...
def test_trajectory_optimization(show_traj=False):
    ...
    # stf = motion_model.State(1.0, 0.0, math.radians(-5), 0)
    ...
    stf = motion_model.State(1.0, 1.0, math.radians(30), 0.0)
    # q0 = np.array([0.0, 5.9642309896174606, -20.907708003006718,
    #                14.901469996355065, 1.0058176398454606])
    q0 = calculate_initial_guess(st0, stf)

    q = optimize_trajectory(st0, stf, q0, 5)
