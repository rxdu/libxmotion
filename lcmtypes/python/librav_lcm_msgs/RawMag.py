...
class RawMag(object):
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(RawMag._get_packed_fingerprint())
    ...
    def decode(data):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != RawMag._get_packed_fingerprint():
            raise ValueError("Decode error")
        return RawMag._decode_one(buf)
    ...
    def _get_packed_fingerprint():
        if RawMag._packed_fingerprint is None:
            RawMag._packed_fingerprint = struct.pack(">Q", RawMag._get_hash_recursive([]))
        return RawMag._packed_fingerprint
